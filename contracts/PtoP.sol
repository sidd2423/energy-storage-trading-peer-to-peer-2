...
contract PtoP is Ownable {
    ...
    ) external view isAdminModifier(adminAddress) returns (bool) {
        return true;
    }
    ...
    ) external isAdminModifier(msg.sender) stopInEmergency returns (uint256) {
    ...
    function changeBatteryThreshold(
        uint256 _batteryID,
        uint256 _newThreshold
    ) external stopInEmergency isAdminModifier(msg.sender) {
    ...
    function decommissionBattery(
        uint256 _batteryID
    ) external stopInEmergency isAdminModifier(msg.sender) returns (uint256) {
        ...
        batteries[batteries.length - 1].mapIndex = batteries[_batteryID]
            .mapIndex;
    ...
    function getRelevantBatteryInfo(
        uint256 _batteryID
    )
        external
        view
        returns (uint256, uint256, bytes32, uint256, uint256, bool, uint256)
    {
        return (
            batteries[_batteryID].capacity,
            batteries[_batteryID].currentFilled,
    ...
    function getBatteryCapacityRemaining(
        uint256 _batteryID

...
contract BatteryInvestment {
    ...
    PtoP internal PtoPContract;
    address public PtoPAddress;
    ...
    modifier isAdmin(address msgAddress) {
        require(PtoPContract.isAdmin(msgAddress) == true);
        _;
    }

    // contructor
    /// @param _PtoPAddress address of parent contract
    /// @param _dividendPercentage set dividend percentage threshold
    constructor(address _PtoPAddress, uint256 _dividendPercentage) public {
    ...
    function updateRemainingInvestment(
        uint256 _remainingInvestment
    ) external isAdmin(msg.sender) returns (bool) {
    ...
    function investMoney() external payable enoughInvestmentModifier {
        ...
        numInvestorInvestments[msg.sender] = numInvestorInvestments[msg.sender]
            .add(1);
    ...
    function withdraw() external payable isValidWithdrawalModifier(msg.sender) {
        ...
        if (numInvestorsWithdraw == investorsList.length) {
            ...
            pendingTotalWithdrawals[0] = 0;
            remainingInvestment = remainingInvestment.add(
                pendingTotalWithdrawals[1]
            );
        }
        ...
        returns (uint256)
    {
        ...
        uint256 totalCurrentDividend = remainingInvestment.mul(
            dividendPercentage
        );
    ...
    function addPendingWithdrawals(
        address currentAddress
    ) external isAdmin(msg.sender) returns (uint256) {
        ...
        pendingWithdrawals[currentAddress] = pendingWithdrawals[currentAddress]
            .add(totalUserDividend);
    ...
    function getInvestorInvestment(
        address investorAddress,
        uint256 index
    ) external view returns (uint256 returnInvestment) {
    ...
    /// @param investorAddress investor address
    function getNumInvestorInvestment(
        address investorAddress
    ) external view returns (uint256 numInvestments) {
    ...
    function getPendingWithdrawal(
        address investorAddress
    ) external view returns (uint256 pendingWithdrawal) {
